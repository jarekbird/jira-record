...
Usage:
  python3 sync_epic_to_jira.py file1.md file2.md [--dry-run] [--no-transition]
  python3 sync_epic_to_jira.py current/Epics/*Epic*.md [--dry-run] [--no-transition]
"""
...
def _extract_problem_statement(md: str) -> str:
    ...
    if problem_match:
        content = problem_match.group(1).strip()
...
def _build_epic_update(record: ParsedEpicRecord, *, allow_network: bool) -> Dict[str, Any]:
    e = record.epic
    fields: Dict[str, Any] = {}
    ...
    elif e.pm_owner and not allow_network:
        fields[CF_PM_OWNER] = {"accountId": f"<unresolved:{e.pm_owner}>"}
    
    # Assignee
    asg_id = jira_find_user_account_id(e.assignee, allow_network=allow_network)
    if asg_id:
        fields["assignee"] = {"accountId": asg_id}
    elif e.assignee and not allow_network:
        fields["assignee"] = {"accountId": f"<unresolved:{e.assignee}>"}
    
    # Description (ADF) - normalize and include if present
...
def _process_single_file(md_path: str, args) -> bool:
    """Process a single Epic .md file and update Jira. Returns True on success, False on failure."""
    try:
    ...
    epic_key = record.epic.jira_key

    epic_fields = _build_epic_update(record, allow_network=can_call_jira and not args.dry_run)

    print(f"\n{os.path.basename(md_path)}:")
    print(f"  Epic: {epic_key}")

    if args.dry_run:
        print("  [dry-run] Epic field updates:")
        print(json.dumps(epic_fields, indent=4, sort_keys=True))
    else:
        # Update Epic fields
        try:
            jira_update_issue_fields(epic_key, epic_fields)
            print(f"  ✓ Updated Epic fields: {epic_key}")
        except JiraApiError as e:
            # If error is about closed sprint, retry without sprint field
            if "can be assigned only active or future sprints" in str(e) and CF_SPRINT in epic_fields:
                print(f"  ⚠ Warning: Sprint is closed, retrying without sprint field", file=sys.stderr)
                epic_fields_no_sprint = {k: v for k, v in epic_fields.items() if k != CF_SPRINT}
                try:
                    jira_update_issue_fields(epic_key, epic_fields_no_sprint)
                    print(f"  ✓ Updated Epic fields (without sprint): {epic_key}")
                except JiraApiError as e2:
                    print(f"  ✗ Failed to update Epic {epic_key}: {e2}", file=sys.stderr)
                    return False
            else:
                print(f"  ✗ Failed to update Epic {epic_key}: {e}", file=sys.stderr)
                return False
    ...
    if not args.no_transition:
        if not can_call_jira:
            ...
            try:
                _maybe_transition(epic_key, record.epic.status, dry_run=args.dry_run)
            except Exception as e:
                print(f"  ⚠ Epic transition warning: {e}", file=sys.stderr)
...
def main():
    parser = argparse.ArgumentParser(
        description="Update Jira Epic from local .md record(s)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Update a single file
  python3 sync_epic_to_jira.py path/to/epic.md

  # Update multiple files
  python3 sync_epic_to_jira.py file1.md file2.md

  # Update all Epic files in a directory
  python3 sync_epic_to_jira.py current/Epics/*Epic*.md
        """
    )
    parser.add_argument(
        "md_files",
