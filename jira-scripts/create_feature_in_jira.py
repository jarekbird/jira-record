#!/usr/bin/env python3
"""
Create Jira Feature issue(s) from local Markdown record(s).

This script reads one or more `.md` file(s) that follow:
  `ga-jira/.cursor/rules/local-jira-records-feature.mdc`

It then creates the Feature issue(s) in Jira via Jira REST API v3 and updates
...
Usage:
  python3 create_feature_in_jira.py file1.md file2.md [--dry-run]
  python3 create_feature_in_jira.py current/Namespacing/Reporting/*Feature*.md [--dry-run]
"""
...
def _build_feature_create_fields(record: ParsedFeatureRecord, *, allow_network: bool) -> Dict[str, Any]:
    """Build fields for creating a Feature issue."""
    f = record.feature
    fields: Dict[str, Any] = {
        "project": {"key": PROJECT_KEY},
        "issuetype": {"id": ISSUE_TYPE_FEATURE},
...
def _build_feature_adf_update_fields(record: ParsedFeatureRecord) -> Dict[str, Any]:
    """Build ADF fields for updating a Feature (after creation)."""
    fields: Dict[str, Any] = {}
    
    # ADF custom fields - normalize and include if present
...
def _rename_file_with_jira_key(file_path: str, jira_key: str) -> Optional[str]:
    ...
    if not re.match(r'^DRAFT\s*-\s*(?:Feature\s*-\s*)?', old_filename, re.IGNORECASE):
        return None
    
    # Extract title (remove DRAFT - Feature - or DRAFT - prefix)
    title = re.sub(r'^DRAFT\s*-\s*(?:Feature\s*-\s*)?', '', old_filename, flags=re.IGNORECASE)
    # Remove .md extension
...
def _process_single_file(md_path: str, args) -> bool:
    """Process a single Feature .md file and create in Jira. Returns True on success, False on failure."""
    try:
    ...
    if record.feature.jira_key:
        print(f"\n{os.path.basename(md_path)}:")
        print(f"  Feature: {record.feature.jira_key} (already exists, skipping)")
        return True
    ...
    create_fields = _build_feature_create_fields(record, allow_network=can_call_jira and not args.dry_run)
    adf_update_fields = _build_feature_adf_update_fields(record)

    if args.dry_run:
        print("  [dry-run] Would create Feature with fields:")
        print(json.dumps(create_fields, indent=4, sort_keys=True))
        if adf_update_fields:
            print("  [dry-run] Would update ADF fields:")
            print(json.dumps(adf_update_fields, indent=4, sort_keys=True))
    else:
        try:
            # Create the Feature
            ...
            if not jira_key:
                print(f"  ✗ Failed to create Feature: no key returned", file=sys.stderr)
                return False
            
            print(f"  ✓ Created Feature: {jira_key}")
            
            # Update ADF custom fields if present
            if adf_update_fields:
                try:
                    jira_update_issue_fields(jira_key, adf_update_fields)
                    print(f"  ✓ Updated ADF fields: {jira_key}")
                except JiraApiError as e:
                    print(f"  ⚠ Warning: Failed to update ADF fields: {e}", file=sys.stderr)
                    # Continue anyway - the Feature was created
...
def main():
    parser = argparse.ArgumentParser(
        description="Create Jira Feature issue(s) from local .md record(s)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Create a single Feature
  python3 create_feature_in_jira.py path/to/feature.md

  # Create multiple Features
  python3 create_feature_in_jira.py file1.md file2.md

  # Create all DRAFT Features in a directory
  python3 create_feature_in_jira.py current/Namespacing/Reporting/*Feature*.md
        """
    )
    parser.add_argument(
        "md_files",
